...
bool cal_intersection(EleFace *tria1, EleFace *tria2,vector<Vector3f*> &point) {
	...
	//judge whether each edge of tria1 is at same side of tria2 plane
	IntersectionStatus inters_status[6];

	for (int i = 0; i < 3; i++) {
		inters_status[i] = getSegmentFaceIntersection(edgevector1[i % 3], edgevector1[(i + 1) % 3], tria2);
	}
	if (inters_status[0] == 0 && inters_status[1] == 0 && inters_status[2] == 0) {
		return false;
	}//three edges is at the same side,don't intersect with tria2

	//judge whether each edge of tria2 is at same side of tria1
	for (int i = 0; i < 3; i++) {
		inters_status[i + 3] = getSegmentFaceIntersection(edgevector2[i % 3], edgevector2[(i + 1) % 3], tria1);
	}
	if (inters_status[3] == 0 && inters_status[4] == 0 && inters_status[5] == 0) {
		return false;
	}

	//judge each edge of t1 intersect with t2 carefully
	int inters_status[6];
	for (int i = 0; i < 6; i++) {
		if (inters_status[i] == 1) {
			if (i < 3) { inters_status[i] = is_intersection(edgevector1[i], edgevector1[i + 1], tria2); }//t1 edge
			else { inters_status[i] = is_intersection(edgevector2[i % 3], edgevector2[(i + 1) % 3], tria1); }//t2 edge
		}
	}

	// if all inters_status are 0, return false
	for (int i = 0; i < 6; i++) {
		if (inters_status[i]) {
			break;
		}
			...
			if (i < 3) {
				for (int j = 3; j < 6; j++) {
					if (inters_status[j] == 2) {//two edge overlap
						if (cal_intersect_point_collinear(edgevector1[i], edgevector1[i + 1], edgevector2[j - 3], edgevector2[j + 1 - 3], point)) { find_answer = true; }
					}
